...
amendments <- readr::read_delim(col_names = TRUE, show_col_types = FALSE, delim = ";", trim_ws = TRUE, file = "
  ...
  5; 14-May-2026; Editing and protocol updates; Update; Revised protocol language and refined ETL conventions with additional implementation clarifications
")
...
calculated_$\text{LogMAR} = -\log_{10}(\text{Snellen visual acuity as decimal})$
...
$\text{LogMAR} = \text{calculated_logMAR} - 0.02 (\text{for each Snellen letter read})$
...
```
After conversion of values to logMAR, the lowest logMAR (corresponding to the best possible recorded visual acuity) for each eye-level (OD/OS/OU), regardless of testing method (at distance or near, with or without pinhole, etc), for a given office visit will be selected for loading into the OMOP CDM (next step).
...
- **Epic and concatenation of line and letters read**: In Epic, the Snellen line and letters read are typically stored in separate Smart data element fields. For example, if the right eye for a patient reads 20/40 +2, the 20/40 is stored in one location and the +2 another location. During the ETL process, these lines need to be matched up. Furthermore, if there are multiple measurements for the right eye, they will exist in a separate row. If there is another measurement for the right eye that reads 20/50 +3, that will exist as a separate line in Epic Clarity. During the ETL process, it will be important to match the +2 to the 20/40 and the +3 to 20/50. Also keep in mind, depending on the documentation patterns of the providers, some might write 20/40+2 in the same text box in which case both the line and letters read will show up in a single Smart data element field.
